...
Public Class GradientButton
    ...
    Private ButtonImage As Drawing.Image
    ...
    Public Overloads Property AutoEllipsis() As Boolean
        Get
            Return MyBase.AutoEllipsis
        End Get
        Set(ByVal value As Boolean)
            MyBase.AutoEllipsis = True
        End Set
    End Property
    ...
    <DefaultValueAttribute(GetType(ContentAlignment), "MiddleCenter")>
    Public Property ContentAlignment() As ContentAlignment
        Get
            Return MyBase.TextAlign
        End Get
        Set(ByVal value As ContentAlignment)
            If MyBase.TextAlign = value Then Exit Property
    ...
    <EditorBrowsable(EditorBrowsableState.Never)>
    <DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)>
    Public Overloads ReadOnly Property FlatAppearance() As FlatButtonAppearance
        Get
    ...
    Public Overloads Property FlatStyle() As FlatStyle
        ...
        Set(ByVal value As FlatStyle)
            If value <> Windows.Forms.FlatStyle.Flat Or value <> Windows.Forms.FlatStyle.System Then Exit Property
    ...
    Public Overloads Property TextImageRelation() As TextImageRelation
        Get
            Return MyBase.TextImageRelation
        End Get
        Set(ByVal value As TextImageRelation)
            If MyBase.TextImageRelation = value Then Exit Property
    ...
    <EditorBrowsable(EditorBrowsableState.Never)>
    <DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)>
    Public Overloads Property UseCompatibleTextRendering() As Boolean
        Get
    ...
    Private Sub GradientButton_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Me.MouseDown
    ...
    Private Sub GradientButton_MouseEnter(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.MouseEnter
    ...
    Private Sub GradientButton_MouseHover(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.MouseHover
        If Not MouseIsOver Then
            MouseIsOver = True
            Me.Invalidate()
        End If
    ...
    Private Sub GradientButton_MouseLeave(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.MouseLeave
        If MouseIsOver Then
            MouseIsOver = False
            Me.Invalidate()
        End If
    ...
    Private Sub GradientButton_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Me.MouseUp
        If MouseIsDown Then
            MouseIsDown = False
            Me.Invalidate()
        End If
    ...
    Private Sub GradientButton_Paint(ByVal sender As Object, ByVal e As System.Windows.Forms.PaintEventArgs) Handles Me.Paint
        ...
        If ButtonImage IsNot Nothing Then
            e.Graphics.DrawImageUnscaled(ButtonImage, 0, 0)
        End If
    End Sub

    Private Function BackBrush(ByVal Rectangle As RectangleF) As Brush
        If BackgroundStyle = BackgroundStyle.Gradient Then
            Dim TopColor As New Classes.ExtendedColor(BackColor)
            Dim BottomColor As New Classes.ExtendedColor(BackColor)

            If MouseIsDown Then
                TopColor.Darken(0.1)
                BottomColor.Lighten(0.1)
            ElseIf MouseIsOver AndAlso (InnerBorder.Type = Border.BorderType.Popup OrElse (InnerBorder.Type = Border.BorderType.None AndAlso OuterBorder.Type = Border.BorderType.Popup)) Then
    ...
    Private Sub CreateImage()
        ...
        ButtonImage = New Bitmap(Me.Width, Me.Height)
        imageGraphics = Graphics.FromImage(ButtonImage)

        imageGraphics.FillRectangle(New SolidBrush(BackColor), 0, 0, Width, Height)

        If OuterBorder.Type <> Border.BorderType.None And OuterBorder.Color <> Color.Transparent Then
            BorderSize = OuterBorder.Size
        End If
        If InnerBorder.Type <> Border.BorderType.None And OuterBorder.Color <> Color.Transparent Then
            BorderSize += InnerBorder.Size
        End If

        BackRect.X = BorderSize + Padding.Left
        BackRect.Y = BorderSize - 1 + Padding.Top
        BackRect.Width = Width - 2 * BorderSize - Padding.Right - Padding.Left
        BackRect.Height = Height - 2 * BorderSize + 1 - Padding.Bottom - Padding.Top
        ...
        Select Case TextImageRelation
            Case Windows.Forms.TextImageRelation.ImageAboveText, Windows.Forms.TextImageRelation.TextAboveImage
                ...
                If _Image.BaseImage IsNot Nothing Then
                    TextSize.Height = BackRect.Height - Image.Height - 15
                Else
            ...
            Case Windows.Forms.TextImageRelation.ImageBeforeText, Windows.Forms.TextImageRelation.TextBeforeImage
                If _Image.BaseImage IsNot Nothing Then
                    TextSize.Width = BackRect.Width - Image.Width - 15
                Else
                    TextSize.Width = BackRect.Width - 10
                End If
                TextSize.Height = BackRect.Height - 10
        ...
        Select Case TextImageRelation
            Case Windows.Forms.TextImageRelation.ImageAboveText, Windows.Forms.TextImageRelation.TextAboveImage
                If _Image.BaseImage IsNot Nothing Then
                    ...
                    End If
                    ContentSize.Height += Image.Height + 5
                End If
                If TextImageRelation = Windows.Forms.TextImageRelation.ImageAboveText Then
                    If _Image.BaseImage IsNot Nothing Then
                        ImageLocation = New Point(CInt((ContentSize.Width - Image.Width) / 2), 0)
                        TextLocation = New Point(CInt((ContentSize.Width - TextSize.Width) / 2), Image.Height + 5)
                    Else
                ...
                Else
                    If _Image.BaseImage IsNot Nothing Then
                        ImageLocation = New Point(CInt((ContentSize.Width - Image.Width) / 2), ContentSize.Height - Image.Height)
                    Else
            ...
            Case Windows.Forms.TextImageRelation.ImageBeforeText, Windows.Forms.TextImageRelation.TextBeforeImage
                ...
                If _Image.BaseImage IsNot Nothing Then
                    If Image.Height > TextSize.Height Then
                        ContentSize.Height = Image.Height
                    End If
                    ContentSize.Width += Image.Width + 5
                End If
                If TextImageRelation = Windows.Forms.TextImageRelation.ImageBeforeText Then
                    If _Image.BaseImage IsNot Nothing Then
                        ImageLocation = New Point(0, CInt((ContentSize.Height - Image.Height) / 2))
                        TextLocation = New Point(Image.Width + 5, CInt((ContentSize.Height - TextSize.Height) / 2))
                    Else
                ...
                Else
                    If _Image.BaseImage IsNot Nothing Then
                        ImageLocation = New Point(ContentSize.Width - Image.Width, CInt((ContentSize.Height - Image.Height) / 2))
                    Else
            ...
            Case Windows.Forms.TextImageRelation.Overlay
                If _Image.BaseImage IsNot Nothing Then
                    ...
                    If Image.Height > TextSize.Height Then
                        ContentSize.Height = Image.Height
                    End If
                    ImageLocation = New Point(CInt((ContentSize.Width - Image.Width) / 2), CInt((ContentSize.Height - Image.Height) / 2))
                Else
        ...
        If ContentSize.Width > 0 AndAlso ContentSize.Height > 0 Then
            ...
            If _Image.BaseImage IsNot Nothing Then
                ContentGraphics.DrawImage(Image, New Rectangle(ImageLocation.X, ImageLocation.Y, Image.Width, Image.Height))
            End If
            For x = 0 To UBound(TextRows)
